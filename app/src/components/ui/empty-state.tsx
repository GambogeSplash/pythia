...
export function EmptyState({ icon, title, description, action }: EmptyStateProps) {
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      ...
      <h3 className="text-body-14 font-semibold text-text-secondary">{title}</h3>
      {description && (
        <p className="mt-1 max-w-xs text-body-12 text-text-quaternary">{description}</p>
      )}
...
export function PythiaBrandMark({ size = 120, className = "" }: { size?: number; className?: string }) {
  return (
...
export function EmptyDashboard({ onAddWidgets }: { onAddWidgets: () => void }) {
  return (
    <div className="flex h-full flex-col items-center justify-center gap-6 px-4">
      <div className="relative">
        <PythiaBrandMark size={120} className="text-signal-green opacity-[0.08]" />
        <div
          className="absolute inset-0 -z-10 blur-3xl"
          style={{ background: "radial-gradient(circle, rgba(0,255,133,0.06) 0%, transparent 70%)" }}
        />
      </div>
      <div className="text-center">
        ...
        <p className="mt-1 max-w-xs text-body-12 text-text-secondary">
          Add widgets to build your custom trading terminal. Drag and arrange them however you like.
        </p>
      </div>
      <button
        onClick={onAddWidgets}
        className="flex h-8 items-center gap-2 rounded-[8px] bg-signal-green px-4 text-body-12 font-semibold text-bg-base-0 transition-colors hover:bg-signal-green/90"
      >
